    ...
    {STEP_TITLES.map((title, idx) => {
      ...
      return (
        <div key={title} className="flex-1 space-y-1">
          ...
          />
          <span
            className={`text-[10px] font-medium block text-center truncate ${isActive ? 'text-blue-400 font-semibold' : 'text-slate-500'
              }`}
          >
            {title}
          </span>
...
/**
 * Interactive 6-Step AI Optimization Wizard Modal Component.
 *
...
export const AiOptimizationWizardModal: React.FC<AiOptimizationWizardModalProps> = ({
  ...
  const fetchStepProposal = useCallback(
    async (step: number, feedback?: string) => {
      setIsLoading(true);
      setErrorMessage(null);
  ...
  useEffect(() => {
    if (isOpen) {
      fetchStepProposal(currentStep);
    }
  }, [isOpen, currentStep, fetchStepProposal]);
  ...
  const handleAcceptStep = () => {
    ...
    if (stepProposal) {
      ...
      if (currentStep === 1 && typeof prop.proposedTitle === 'string') {
        updated.personalDetails.professionalTitle = prop.proposedTitle;
      } else if (currentStep === 2 && typeof prop.proposedSummary === 'string') {
        updated.summary = prop.proposedSummary;
      } else if (currentStep === 3 && Array.isArray(prop.proposedExperience)) {
      ...
      } else if (currentStep === 5 && Array.isArray(prop.proposedSkills)) {
        updated.skills = prop.proposedSkills as SkillCategory[];
      } else if (currentStep === 6 && Array.isArray(prop.proposedProjects)) {
  ...
  const handleRepropose = () => {
    fetchStepProposal(currentStep, userFeedback);
  };
        ...
        <div className="p-6 flex-1 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="p-4 rounded-lg bg-red-950/40 border border-red-500/30 text-xs text-red-300 space-y-2">
              ...
              <button
                type="button"
                onClick={() => fetchStepProposal(currentStep, userFeedback)}
                className="px-3 py-1 bg-red-500/20 text-red-200 hover:bg-red-500/30 rounded font-medium transition-colors"
              >
                Retry
              </button>
            </div>
